...
namespace SteamPP.Services
{
    ...
    public class SteamKitAppInfoService
    {
        ...
        /// <summary>
        /// Get app info for multiple apps in batches
        /// </summary>
        public async Task<Dictionary<uint, AppInfoResult>> GetAppInfoBatchAsync(IEnumerable<uint> appIds)
        {
            var results = new Dictionary<uint, AppInfoResult>();
            ...
            try
            {
                ...
                var uniqueAppIds = appIds.Distinct().ToList();
                _logger.Info($"[SteamKit] Requesting app info for {uniqueAppIds.Count} apps...");

                foreach (var chunk in uniqueAppIds.Chunk(200))
                {
                    await ProcessAppInfoChunkAsync(chunk, results);
                }

                _logger.Info($"[SteamKit] Successfully retrieved info for {results.Count} apps");
                return results;
            }
            catch (Exception ex)
            {
                _logger.Error($"[SteamKit] Error in batch request: {ex.Message}");
                return results;
            }
        }
        ...
        private async Task ProcessAppInfoChunkAsync(uint[] chunk, Dictionary<uint, AppInfoResult> results)
        {
            try
            {
                var requests = chunk.Select(id => new SteamApps.PICSRequest(id)).ToList();
                
                var requestTask = _steamApps!.PICSGetProductInfo(requests, new List<SteamApps.PICSRequest>()).ToTask();
                var completedTask = await Task.WhenAny(requestTask, Task.Delay(45000));

                if (completedTask != requestTask)
                {
                    _logger.Error("[SteamKit] Batch app info request timed out");
                    return;
                }
                ...
                if (response?.Results != null)
                {
                    ...
                    {
                        foreach (var kvp in result.Apps)
                        {
                            results[kvp.Key] = ParseAppInfo(kvp.Key, kvp.Value);
                        }
                    }
                }
            }
            ...
            {
                _logger.Error($"[SteamKit] Error processing batch chunk: {ex.Message}");
            }
        }
        ...
        private AppInfoResult ParseAppInfo(uint appId, SteamApps.PICSProductInfoCallback.PICSProductInfo appInfo)
        {
            ...
            try
            {
                ...
                // Get last updated time from depots -> branches -> public -> timeupdated
                var branches = depots["branches"];
                if (branches != KeyValue.Invalid)
                {
                    var publicBranch = branches["public"];
                    if (publicBranch != KeyValue.Invalid)
                    {
                        var timeUpdated = publicBranch["timeupdated"];
                        if (timeUpdated != KeyValue.Invalid)
                        {
                            var unixTime = timeUpdated.AsLong();
                            if (unixTime > 0)
                            {
                                result.LastUpdated = DateTimeOffset.FromUnixTimeSeconds(unixTime).DateTime;
                                _logger.Info($"[SteamKit] Last Updated: {result.LastUpdated}");
                            }
                        }
                    }
                }
