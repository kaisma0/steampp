...
namespace SteamPP.Services
{
    ...
    public class DepotDownloaderWrapperService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // Prevent concurrent downloads due to static state in ContentDownloader
            if (System.Threading.Interlocked.CompareExchange(ref _activeDownloadCount, 1, 0) == 1)
            {
                var msg = "Another download is already in progress. Concurrent downloads are not supported.";
                LogInfo(msg);
            ...
            {
                System.Threading.Interlocked.Exchange(ref _activeDownloadCount, 0);
            }
        }
